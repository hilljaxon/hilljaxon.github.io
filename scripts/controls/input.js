Game.input = (function() {
    ...
    function Keyboard() {
        ...
        function findExistingHandler(handler){
            for(let i=0;i<that.handlers.length;i++){
        ...
        that.registerCommand = function(key, handler) {
            let existingKey = findExistingKey(key),
                existingHandler = findExistingHandler(handler);
            if(existingHandler != -1){
                that.handlers.splice(existingHandler,1);
            }
            if(existingKey != -1){
                that.handlers.splice(existingKey,1);
            }
